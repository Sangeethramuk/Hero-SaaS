      ...
      <div className="flex-1 flex flex-col h-full bg-background/50 z-10">
         ...
         <header className="h-16 border-b border-border/40 backdrop-blur-xl px-4 flex items-center justify-between flex-shrink-0 z-20">
             <div className="flex items-center gap-4">
                ...
                <Avatar size="sm" className="bg-gradient-to-br from-warning/80 to-warning text-white font-semibold text-xs">
                   <Avatar.Fallback>{selectedSub?.studentName.charAt(0) || "?"}</Avatar.Fallback>
                </Avatar>
                <div>
                   <h1 className="text-sm font-bold tracking-tight text-foreground leading-none">{selectedSub?.studentName} <span className="font-normal text-muted ml-1">Appeal Review</span></h1>
               ...
               <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div>
                     ...
                     <div className="flex gap-4 h-[300px]">
                        {/* AI Extraction side */}
                        <div className="flex-1 bg-default/20 border border-border/40 rounded-xl p-5 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="flex-1 bg-surface border border-border/40 rounded-xl p-5 flex flex-col items-center justify-center text-center">
                  ...
                  <div className="opacity-50 block w-full h-[1px] bg-border/40" />
                   ...
                   {/* Resolution Action Panel (Screen 15) */}
                    <div className="flex items-center justify-between bg-surface border border-border/50 rounded-xl p-6 shadow-sm">
                       <div>
                       ...
                       <div className="flex items-center gap-3">
                         <Button 
                            variant="ghost" 
                            className={`font-medium px-4 rounded-lg flex items-center gap-2 ${appeal.status === 'Resolved' ? 'opacity-50 pointer-events-none' : 'bg-surface border border-border/50 hover:bg-danger/10 hover:text-danger hover:border-danger/30'}`}
                            onPress={() => {
                         ...
                         <Button 
                            variant="primary" 
                            className={`font-medium px-4 rounded-lg shadow-md flex items-center gap-2 ${appeal.status === 'Resolved' ? 'opacity-50 pointer-events-none bg-default text-muted' : 'bg-success text-white shadow-success/20'}`}
                            onPress={() => {
                         ...
                         <div className="w-[1px] mx-2 h-6 bg-border/50" />
                          <Button 
                             variant="ghost" 
                             className={`font-medium px-4 rounded-lg flex items-center gap-2 ${appeal.status === 'Resolved' ? 'opacity-50 pointer-events-none' : 'bg-accent/10 border border-accent/20 text-accent hover:bg-accent/20'}`}
                             onPress={() => router.push(`/grading/${selectedSubId}`)}
                          >
                             <Sliders className="size-4" /> Open Grading Desk to Adjust
                          </Button>
                      </div>
                   </div>
